...
$pdf = new PDF();
$pdf->setTitle('Faculty Manual Revisions');
$pdf->AliasNbPages();
$pdf->AddPage();
$rows = $crud->getData("SELECT sectionNo, title, content FROM facultyassocnew.sections ORDER BY sectionNo;");
foreach((array)$rows AS $key => $row){
    $pdf->SetFont('Times','B',18);
    $pdf->Cell(0,10,'Section '.$row['sectionNo'].' - '.$row['title'],0,1);
    $pdf->SetFont('Times','',12);
    $pdf->WordWrap($row['content'],'180');
    $pdf->Write(5,$row['content']);
    //$pdf->Justify($row['content'],85,4);
    $pdf->Ln(10);
}
$pdf->Output();
?>