<?php
/**
 * Created by PhpStorm.
 * User: Serus Caligo
 * Date: 10/4/2018
 * Time: 3:31 PM
 */
?>
<!-- Sidebar Menu Items - These collapse to the responsive navigation menu on small screens -->
    <div class="collapse navbar-collapse navbar-ex1-collapse">

        <ul class="nav navbar-nav side-nav">
            <br>
            <!--
            <form>
                <div class="form-group">
                    <input class="form-control" placeholder="Search">
                </div>
            </form>

            <li class="active">
                <a href="javascript:;" data-toggle="collapse" data-target="#demo"><i class="fa fa-fw fa-folder-open"></i> Documents <i class="fa fa-fw fa-caret-down"></i></a>
                <ul id="demo" class="collapse"> -->
            <li>
                <a href="EDMS_Applications.php"><i class="fa fa-fw fa-folder"></i> Applications</a>
            </li>
            <li>
                <a href="EDMS_ByLaws.php"><i class="fa fa-fw fa-folder"></i> By-Laws</a>
            </li>
            <li>
                <a href="EDMS_Contracts.php"><i class="fa fa-fw fa-folder"></i> Contracts</a>
            </li>
            <li>
                <a href="EDMS_FacultyManual.php"><i class="fa fa-fw fa-folder"></i> Faculty Manual</a>
            </li>
            <li>
                <a href="javascript:;" data-toggle="collapse" data-target="#minutes"><i class="fa fa-fw fa-folder"></i> Minutes <i class="fa fa-fw fa-caret-down"></i></a>
                <ul id="minutes" class="collapse">
                    <li style="">
                        <a href="#"><i class="fa fa-fw fa-folder"></i> Executive Board Minutes</a>
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-fw fa-folder"></i> General Assembly Minutes</a>
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-fw fa-folder"></i> Technical Panel Minutes</a>
                    </li>
                </ul>
            </li>
        </ul>
    </div>
    <!-- /.navbar-collapse -->
</nav>
<!-- END OF NAV -->
