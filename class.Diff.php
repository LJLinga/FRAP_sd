...
class Diff{
   ...
   * $compareCharacters is true), and one aof the constants DIFF::UNMODIFIED (the
  ...
  public static function toHTML($diff, $separator = '<br>'){
    ...
    foreach ($diff as $line){
      ...
      switch ($line[1]){
        case self::UNMODIFIED : $element = 'span class="unmodified-text"'; break;
        case self::DELETED    : $element = 'span class="removed-text"';  break;
        case self::INSERTED   : $element = 'span class="added-text"';  break;
      }
      $html .=
          '<' . $element . '>'
  ...
  public static function toTable($diff, $indentation = '', $separator = '<br>'){
    ...
    // initialise the HTML
    $html = $indentation . '<table class="diff table-condensed table-sm" width="100%">';
  ...
  private static function getCellContent(
    ...
    while ($index < count($diff) && $diff[$index][1] == $type){
      $html .=
          '<span>'
          //. htmlspecialchars($diff[$index][0]) <-- Removed by Christian, this puts html tags to content.
          . $diff[$index][0]
          . '</span>'
          . $separator;
      $index ++;
