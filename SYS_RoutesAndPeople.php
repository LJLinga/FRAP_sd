...
$page_title = 'Configuration - Routes and People';
...
if(isset($_GET['id'])){
    ...
    $rows = $crud->getData("SELECT stepName, stepNo FROM steps s WHERE s.id = '$stepId'");
    foreach((array)$rows AS $key => $row){
        $stepName = $row['stepName'];
        $stepNo = $row['stepNo'];
    }

    $rows = $crud->getData("SELECT * FROM facultyassocnew.step_routes WHERE currentStepId = '$stepId' LIMIT 1;");
    foreach((array)$rows AS $key => $row){
